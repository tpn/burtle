// Implementation of Jasper, a 128-bit noncryptographic hash
...
// but it's used by Jasper just for short messages.
void ShortHash(
  const void *message,
  size_t length,
  uint64_t *hash1,
  ...
  uint64_t *hash2)
{
  if (length < BLOCKSIZE) {
    ...
    uint64_t h0,h1,h2,h3,h4,h5,h6,h7,h8,h9,h10,h11, temp;
    uint64_t buf[12];
    ...
    for (; data < end; data+=12) { 
      JasperMix(temp, data, h0,h1,h2,h3,h4,h5,h6,h7,h8,h9,h10,h11);
    }
    ...
    JasperMix(temp, buf, h0,h1,h2,h3,h4,h5,h6,h7,h8,h9,h10,h11);
...
// init jasper state
void JasperInit(Jasper *state, uint64_t seed1, uint64_t seed2)
{
  state->length = 0;
  state->remainder = 0;
  state->state[0] = seed1;
...
void JasperUpdate(Jasper *state, const void *message, uint64_t length)
{
  uint64_t h0,h1,h2,h3,h4,h5,h6,h7,h8,h9,h10,h11, temp;
...
void JasperFinal(Jasper *state, uint64_t *hash1, uint64_t *hash2)
{
  uint64_t h0,h1,h2,h3,h4,h5,h6,h7,h8,h9,h10,h11, temp;
  const uint64_t *data;
  const uint64_t *end;
