...
void DoTiming()
{
  ...
  for (j=0; j<200; ++j) {
    a = GetTickCount();
    for (i=0; i<NUMBUF; ++i) {
      SpookyHash(buf[i], BUFSIZE, &hash1, &hash2);
    }
    z = GetTickCount();
...
void TestPair()
{
  ...
  for (h=0; h<BUFSIZE; ++h) {  // for messages 0..255 bytes
	...
	for (k=0; k<TRIES; ++k) { // try to hit every output bit TRIES times
	  ...
	  SpookyHash(buf1, h, &measure[0][0], &measure[0][1]);
	  SpookyHash(buf2, h, &measure[1][0], &measure[1][1]);
	  for (l=0; l<2; ++l) {
...
int main()
{
  ...
  for (i=0; i<256; ++i) {
    ...
    // all as one call
    SpookyHash(buf, i, &a, &b);

    // all as one piece
    SpookyInit(&state, c, d);
    SpookyUpdate(&state, buf, i);
    SpookyFinal(&state, &c, &d);
    
    // a bunch of 1-byte pieces
    SpookyInit(&state, e, f);
    for (j=0; j<i; ++j) {
      SpookyUpdate(&state, &buf[j], 1);
    }
    SpookyFinal(&state, &e, &f);
