  ...
  uint64_t *hash2)
{
  if (length < BLOCKSIZE) {
    ...
    for (; data < end; data+=12) { 
      AkronMix(data, h0,h1,h2,h3,h4,h5,h6,h7,h8,h9,h10,h11);
    }
    ...
    AkronMix(buf, h0,h1,h2,h3,h4,h5,h6,h7,h8,h9,h10,h11);
...
// init akron state
void AkronInit(Akron *state, uint64_t seed1, uint64_t seed2)
{
  state->length = 0;
  state->remainder = 0;
  state->state[0] = seed1;
...
void AkronUpdate(Akron *state, const void *message, uint64_t length)
{
  uint64_t h0,h1,h2,h3,h4,h5,h6,h7,h8,h9,h10,h11;
...
void AkronFinal(Akron *state, uint64_t *hash1, uint64_t *hash2)
{
  uint64_t h0,h1,h2,h3,h4,h5,h6,h7,h8,h9,h10,h11;
  const uint64_t *data;
  const uint64_t *end;
