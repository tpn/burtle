...
Arguments
  ...
  dimension.  Dimensions are implicitly numbered 1..65535, in the order
  they appear.  Features in dimensions are always implicitly given
  1-character names, which are in order a..z, A..Z .  It's a good idea
...
int insert_tuple( tu_iter *ctx, feature *tuple)
{
  ub4      i;
  feature *fe;
  ub1      n = ctx->n;
  ...
  if (!*ctx->tu) {
    if (!((*ctx->tu) = (tu_arr *)bob_malloc(sizeof(tu_arr)))) {
      return FALSE;
    }
    (*ctx->tu)->len = 0;
...
void build_tuples( state *s, ub2 d, ub2 f)
{
  ...
  for (;;) {
    /* only accept tuples that contain dimension d with feature f */
    /* Slow.  I ought to generate (n-1)-tuples and insert d.f into them */
    for (i=0; i<n; ++i) {
      if (offset[i].d == d && offset[i].f == f)
	break;
    }
    if (i==n)
    ...
    for (i=0; i<n; ++i)
      printf("%d%c ",
	     offset[i].d+1, feature_name[offset[i].f]);
    printf("\n");
#endif
...
void cover_tuples( state *s)
{
  ...
  while (TRUE) {
	...
	for (f=0; f<s->dim[d]; ++f) {
      ...
      bob_free((char *)best_test->f);
      bob_free((char *)best_test);
    } else {
      ub2      d;
      for (d=0; d<s->ndim; ++d) {
	tu_iter  ctx;
...
void driver( int argc, char *argv[])
{
  ...
  if (parse(argc, argv, &s)) {               /* read the user's instructions */
    preliminary(&s);                          /* do any preliminary analysis */
