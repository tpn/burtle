...
static void c_do_one_weave(/*_ weave *oldweave, weave *newweaves _*/);
...
void       c_handle(list, thisweave, newweaves)
...
static void c_do_one_weave(oldweave, newweaves)
...
------------------------------------------------------------------------------
*/
void       c_follow(l, crossings, answer)
instruct  *l;
word       crossings;
poly     **answer;
{
  ...
  for (i = 0; i < crossings; i++)
  {
    ...
    printf("cross section: %d  max weaves: %ld  total weaves handled: %ld\n",
           newcross, newfact, total_weaves_handled);
  }
...
main()
{
  ...
  if (!k_read(crossings, knot))                         /* read in the link */
  {
    return FALSE;
  }
